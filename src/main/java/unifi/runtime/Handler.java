 ...
 *
 * <p>Scheduling messages is accomplished with the {@link #post}, {@link
 * #postAtTime(Runnable, long)}, {@link #postDelayed}, {@link
 * #sendEmptyMessage}, {@link #sendMessage}, {@link #sendMessageAtTime}, and
 * {@link #sendMessageDelayed} methods.  The <em>post</em> versions allow you to
 * enqueue Runnable objects to be called by the message queue when they are
 * received; the <em>sendMessage</em> versions allow you to enqueue a {@link
 * Message} object containing a bundle of data that will be processed by the
 * Handler's {@link #handleMessage} method (requiring that you implement a
 * subclass of Handler).
 *
 * <p>When posting or sending to a Handler, you can either allow the item to be
 ...
 *
 * <p>When a process is created for your application, its main thread is
 * dedicated to running a message queue that takes care of managing the
 * top-level application objects (activities, broadcast receivers, etc) and any
 * windows they create.  You can create your own threads, and communicate back
 * with the main application thread through a Handler.  This is done by calling
 * the same <em>post</em> or <em>sendMessage</em> methods as before, but from
 * your new thread.  The given Runnable or Message will then be scheduled in the
...
public class Handler {
  ...
  /**
   * Constructor associates this handler with the {@link Looper} for the current
   * thread and takes a callback interface in which you can handle messages.
   *
   * If this thread does not have a looper, this handler won't be able to
   * receive messages so an exception is thrown.
   *
  ...
  /**
   * Returns a new {@link unifi.runtime.Message Message} from the global message
   ...
   * {@link SystemClock#millisTime()}.</b> Time spent in deep sleep will add
   * an additional delay to execution. The runnable will be run on the thread to
   * which this handler is attached.
   *
   * @param r            The Runnable that will be executed.
   * @param uptimeMillis The absolute time at which the callback should run,
   *                     using the {@link SystemClock#millisTime()} time-base.
   *
   * @return Returns true if the Runnable was successfully placed in to the
   * message queue.  Returns false on failure, usually because the looper
   * processing the message queue is exiting.  Note that a result of true does
   * not mean the Runnable will be processed -- if the looper is quit before the
   ...
   * which this handler is attached. <b>The time-base is
   * {@link SystemClock#millisTime()}.</b> Time spent in deep sleep will add
   * an additional delay to execution.
   *
   * @param r           The Runnable that will be executed.
  ...
  /**
   * Posts a message to an object that implements Runnable. Causes the Runnable
   * r to executed on the next iteration through the message queue. The runnable
   * will be run on the thread to which this handler is attached. <b>This method
  ...
  /**
   * Runs the specified task synchronously. <p> If the current thread is the
   * same as the handler thread, then the runnable runs immediately without
   * being enqueued.  Otherwise, posts the runnable to the handler and waits for
   * it to complete before returning. </p><p> This method is dangerous!
   * Improper use can result in deadlocks. Never call this method while any
   * locks are held or use it in a possibly re-entrant manner. </p><p> This
   * method is occasionally useful in situations where a background thread must
   * synchronously await completion of a task that must run on the handler's
   * thread.  However, this problem is often a symptom of bad design. Consider
   * improving the design (if possible) before resorting to this method. </p><p>
   * One example of where you might want to use this method is when you just set
   * up a Handler thread and need to perform some initialization steps on it
   * before continuing execution. </p><p> If timeout occurs then this method
   * returns <code>false</code> but the runnable will remain posted on the
   * handler and may already be in progress or complete at a later time. </p><p>
   ...
   * blocking runnables.) </p>
   *
   * @param r       The Runnable that will be executed synchronously.
   * @param timeout The timeout in milliseconds, or 0 to wait indefinitely.
   *
   * @return Returns true if the Runnable was successfully executed. Returns
   * false on failure, usually because the looper processing the message queue
   * is exiting.
   *
   * @hide This method is prone to abuse and should probably not be in the API.
   * If we ever do make it part of the API, we might want to rename it to
   * something less funny like runUnsafe().
   */
   ...
   * messages before the current time. It will be received in {@link
   ...
   * processing the message queue is exiting.
   *
   * @see #sendMessageDelayed(unifi.runtime.Message, long)
   */
   ...
   * processing the message queue is exiting.
   *
   * @see #sendMessageAtTime(unifi.runtime.Message, long)
   */
   ...
   * (current time + delayMillis). You will receive it in {@link
   * #handleMessage}, in the thread attached to this handler.
   *
   * @return Returns true if the message was successfully placed in to the
   * message queue.  Returns false on failure, usually because the looper
   * processing the message queue is exiting.  Note that a result of true does
   * not mean the message will be processed -- if the looper is quit before the
   * delivery time of the message occurs then the message will be dropped.
   */
  ...
  /**
   * Enqueue a message into the message queue after all pending messages before
   * the absolute time (in milliseconds) <var>uptimeMillis</var>. <b>The
   * time-base is {@link SystemClock#millisTime()}.</b> Time spent in
   * deep sleep will add an additional delay to execution. You will receive it
   * in {@link #handleMessage}, in the thread attached to this handler.
   *
   * @param uptimeMillis The absolute time at which the message should be
   *                     delivered, using the {@link SystemClock#millisTime()}
   *                     time-base.
   *
   * @return Returns true if the message was successfully placed in to the
   * message queue.  Returns false on failure, usually because the looper
   ...
   * next iteration of the message loop.  You will receive it in {@link
   * #handleMessage}, in the thread attached to this handler. <b>This method is
   * only for use in very special circumstances -- it can easily starve the
   * message queue, cause ordering problems, or have other unexpected
   * side-effects.</b>
   *
   * @return Returns true if the message was successfully placed in to the
  ...
  private boolean enqueueMessage(MessageQueue queue, Message msg, long uptimeMillis) {
    ...
    return queue.enqueue(msg, uptimeMillis);
  }
  ...
  /**
   * Check if there are any pending posts of messages with code 'what' and whose
   * obj is 'object' in the message queue.
   */
  ...
  public final void dump(PrintWriter pw, String prefix) {
    pw.println(prefix + this + " @ " + SystemClock.millisTime());
    if (mLooper == null) {
  ...
  private static final class BlockingRunnable implements Runnable {
    ...
    public boolean postAndWait(Handler handler, long timeout) {
      ...
      synchronized (this) {
        if (timeout > 0) {
          final long expirationTime = SystemClock.millisTime() + timeout;
          while (!mDone) {
            long delay = expirationTime - SystemClock.millisTime();
            if (delay <= 0) {
