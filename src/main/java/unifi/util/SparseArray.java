...
public class SparseArray<E> implements Cloneable {
  ...
  public SparseArray(int initialCapacity) {
    if (initialCapacity == 0) {
      ...
      mValues = ArrayUtils.EMPTY_OBJECT_ARRAY;
    } else {
  ...
  /**
   * Gets the Object mapped from the specified key, or <code>null</code> if no such mapping has been
   * made.
   */
  ...
  public void put(int key, E value) {
    ...
    if (i >= 0) {
      ...
      mKeys = ArrayUtils.add(mKeys, i, key);
      mValues = ArrayUtils.add(mValues, i, value);
      mSize++;
    }
  }
   ...
   * order, e.g., <code>keyAt(0)</code> will return the smallest key and
   ...
   * number if no keys map to the specified value. <p>Beware that this is a linear search, unlike
   * lookups by key, and that multiple keys can map to the same value and this will find only one of
   * them. <p>Note also that unlike most collections' {@code indexOf} methods, this method compares
   * values using {@code ==} rather than {@code equals}.
   */
  ...
  public void append(int key, E value) {
    ...
    mKeys = ArrayUtils.add(mKeys, key);
    mValues = ArrayUtils.add(mValues, value);
    mSize++;
  }
